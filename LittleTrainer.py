...
def set_checkpoint_log_save_lossacc_interval(iter_per_epoch,config_train):
    ...
    if config_train.log_interval < 1:
        config_train.log_interval = 1
    if config_train.log_interval > iter_per_epoch:
...
def step_log_savemodel_checkpoint(epoch,step,iter_per_epoch,start_time,loss,optimizer,
    ...
    if (step + 1) % config_train.checkpoint_interval == 0:
...
def get_batch_logps(logits,labels,mask):
    ...
    # 根据label的索引拿到chosen和rejected的对应概率（logPx,logQx)
...
def set_train_args(parser):
    ...
    parser.add_argument("--logcnt", type=int, default=100, help="日志次数 为0时使用默认log_interval")
    parser.add_argument("--savecnt", type=int, default=10, help="保存次数 为0时使用默认save_interval")
    parser.add_argument("--num_checkpoint", type=int, default=1, help="checkpoint次数 为0时使用默认checkpoint_interval")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_id", type=str, default='20250613001', help="训练id")
    parser.add_argument("--mode", type=str, default='sft', help="pre:预训练,sft:微调,dpo:偏好dpo,orpo:sft+偏好orpo")
